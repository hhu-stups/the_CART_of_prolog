:- module(cart_regression_sic, [residual_sum_sq/3, split_class/4, mean/2, residual_split/3,
    impurity/3, best_split/3, get_value/3, get_values_n/3,get_values_k/3, 
    my_powerset/3, combination/3, all_combinations/4,create_candidates/3,
    split_candidates_attribute/3, split_candidates/3,find_best_split/3,
    split/4, induce_tree/3, check_sample/3, check_all_sample/3]).
...
check_all_sample(_, [], []).
check_all_sample(Tree, [H|T], [Pred|Preds]) :-
    check_sample(Tree, H, Pred),
    check_all_sample(Tree, T, Preds).
...
my_powerset(Values, Attribut, Zaehler, N, Acc, Kandiaten) :-
    ...
    prefix_length(Res, FirstElements, ZuNehmen),
