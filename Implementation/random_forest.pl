:- module(random_forest, [check_sample_forest/3, majority/2, induce_forest/4,
    bootstrappen/3, choose_attributes/3, induce_trees/3, get_sample/3,
    choose_all_members/4, check_forest/3, check_sample_forest/3, check_all_sample_forest/3]).
...
%! check_all_sample_forest(+RandomForest:Entscheidungswald, +Examples:list, -Res:list) is det
%
%  Gibt zu jedem Datenpunkt aus der Liste eine Vorhersage vom Entscheidungswald aus
check_all_sample_forest(RandomForest, Examples, Res) :-
    maplist(check_sample_forest(RandomForest), Examples, Res).
...
%! check_forest(+RandomForest:list, +Example:list, -Ergebnisse:list) is det
%
